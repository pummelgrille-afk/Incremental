...
import { grantStartingLoadout, mountChime } from '../src/lib/progression/loadout'
...
describe('each trigger', () => {
  ...
  it('Within Tolerance — is not earned by an untouched *loss*', () => {
    // The flag only means anything on a clear; a stage lost at full Tension is
  ...
  it('Documented Procedure — needs a zone cleared with no Chime ever mounted', () => {
  ...
  it('Documented Procedure — is denied by a Chime mounted earlier in the run', () => {
    /*
    ...
    save.meta.keys = 100
    save.run.filings = 10_000
    unlock(save, 'chime', 'quarter-bell')
    mountChime(save, 'quarter-bell', 0)
  ...
  it('The Undermaster Will Hear of This — needs a loss with everything owned fielded', () => {
    save.meta.movements = { hammer: 1, detent: 1 }

    // Only one of the two types on the field.
    evaluate(save, 'stage-lost', { distinctMovementsSlotted: 1, unlockedMovements: 2 })
    expect(isEarned(save, 'the-undermaster-will-hear-of-this')).toBe(false)

    evaluate(save, 'stage-lost', { distinctMovementsSlotted: 2, unlockedMovements: 2 })
  ...
  it('The Undermaster — is not earned by a *clear* with everything fielded', () => {
    save.meta.movements = { hammer: 1 }
    evaluate(save, 'stage-cleared', { distinctMovementsSlotted: 1, unlockedMovements: 1 })
    expect(isEarned(save, 'the-undermaster-will-hear-of-this')).toBe(false)
  })

  it('The Undermaster — is not earned by owning nothing', () => {
    // A player with an empty roster trivially has "all of it" fielded.
    evaluate(save, 'stage-lost', { distinctMovementsSlotted: 0, unlockedMovements: 0 })
    expect(isEarned(save, 'the-undermaster-will-hear-of-this')).toBe(false)
  })
