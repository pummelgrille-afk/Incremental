 ...
 *
 * **Salvage buy the size of a formation, not each placement.** Growing costs
 ...
 * §2 makes for a free respec. What Salvage gate is *how large a machine you
...
export interface PlacementResult {
  placed: boolean
  /** Salvage spent. Zero when moving a unit already in the formation. */
  spent: number
...
/**
 * Put a Platform in a slot, charging only if the formation grew.
 *
...
/**
 * Take a Platform out, refunding what the slot cost.
 *
...
export function mountArray(save: SaveData, defId: string, mount: number): PlacementResult {
  ...
  // Sticky for the whole run — see `arraysEverMounted` in saveSchema.ts.
...
export interface LoadPresetResult {
  loaded: boolean
  /** Net Salvage change. Negative when the preset is larger than the current field. */
  spent: number
 ...
 *
 * **Four Platforms, two on ring 1 and two on ring 2.** Both halves of that were
 * measured, not chosen:
 *
 ...
 * Flare, three lose 8 of 16, four clear all of them. The pillar is the reason
 * the Flare exists as upside rather than as a tax, so it wins over the slot
 * curve's first four purchases.
 *
 * *Two rings*, because coverage matters more than count at this size: four
 * Hammers all on ring 2 lose 24 of 24 without the Flare, while two-and-two clear
 * 24 of 24 at 0.78 Output. Ring 1 is the last line — the only ring that
 * reaches the Sun itself. Splitting them also means conjunction can fire
 * in the first stage, since it requires two Platforms on *different* rings.
 *
 * The slot curve is untouched: the granted four count toward `slotsUsed`, so
 * the fifth Platform costs what the fifth Platform costs. The player is given a
 ...
 * units, no Salvage, and Salvage only come from kills.
 */
